...
- [x] Executive Dashboard — live CW data, recharts charts, dept cards, utilization bars (commit 241d3f6)
- [x] Design System — KPICard, KPIStrip, SectionHeader reusable components (commit 6bb1b97)
- [x] Financials Dashboard — /financials with live invoices, agreements, POs, revenue/aging charts (commit 5343a2b)
- [x] Departments Dashboard — /departments with live CW data, dept cards, utilization chart (commit 28280e0)
- [x] Ticket Table inline badges — consolidated 6 columns to 3, priority/status/assignee inline (commit b6bd213)
- [x] Compact KPI cards — horizontal layout ~56px, consolidated 3 custom implementations into 1 shared component (commit 0d07ef2)
- [x] Sidebar Tools — expandable Tools section with 14 external tool links (commit d94c703)
- [x] Ticket Detail redesign — action bar consolidation, merged Quick Actions + Remote Tools (commit c7d2374)
- [x] Schedule & Dispatch fixes — correct ticket links, light/dark calendar CSS, dept/member filters (commit c7d2374)
- [x] Global Search rewrite — inline expandable search pill, Ctrl+K, categorized results, keyboard nav (commit c7d2374)
- [x] Projects Kanban — Completed column with green accent, collapsible after 5 (commit c7d2374)
- [x] CBR Dashboard — ScalePad-powered Client Business Review: BFF client + normalizers + health score engine, 7 API routes, 8 UI components (client list, detail, hardware health, license optimization, opportunity pipeline, contracts, initiatives), sidebar nav for AM/LT (commit 3ecf8ce)
- [x] SharePoint Document Hub — Microsoft Graph integration (MSAL app-only auth), document browser UI with breadcrumb nav, preview panel, search, 3 BFF routes, LT sidebar nav (commit df9db26)
